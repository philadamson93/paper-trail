...
| [`isolation-protocol.md`](isolation-protocol.md) | **Draft — Codex review Blocked (6 criticals), ALL APPLIED 2026-09-13**; containers-first decided 2026-09-12 · **second explain-plan round applied 2026-09-14 — ALL NINE open questions now resolved** · 1,861 lines (was 1,494) · 5 phases in build order · **scope cut, not grown**: driver session eliminated (OQ1), per-run git clone replaced by a run-start reset (OQ9), tag-scoped settings file retired (OQ6); one small engine change added (OQ2, `val_inputs` per-iteration) · read **What we are building** for the concrete 12-item list · branch from Plan A's tip `f02d761`, lands after it | No | Put **both** principals inside `agentic-label-opt`'s Docker substrate — the mount set is the boundary, read for the judge and write for the optimizer; prove it with a sentinel the judge cannot read plus a control that must fail. The review corrected the biggest premise: the consumer-side container agent is **ours**, not the engine's owed step, and paper-trail has no engine dependency at all yet |
