...
`Reviewed` values: `Yes` (Phil approved the plan's **rendered explainer**, and that explainer is in
sync with the plan's current content) · `No` (never) · `Stale` (was `Yes`, edited substantively
since). ⚠ **An approved, in-sync `/explain-plan` HTML is the only thing that promotes a row to
`Yes`.** `/read-plan` no longer records a review — it opens the markdown, and Phil does not read
markdown, so opening it never showed him the plan in a form he reads.
...
| [`isolation-protocol.md`](isolation-protocol.md) | **BUILT 2026-09-19 — every phase implemented, verified and pushed** on `sarol-optimizer-concurrent` (tip `8562695`). Phases 0, 1, 1g, 2, 3 and 4 all green: the dispatch runs in a container, the seal is proved with sentinels against a live daemon, the program is held to the version being scored, the configuration is pinned in the manifest and refused before spending, and run state is archived and asserted clean at run start. 613 checks across nine suites plus the freeze and run-scope gates; 33 of 33 applicable mutations caught. Seven review passes: fresh-Claude, Codex ×3, Phil ×2, a four-agent live-code audit · all **14** open questions resolved · 2,869 lines · scope: **adjudicator dispatch only** (the eval harness's single stage — the product runs all three) · ⚠ **Reviewed: STALE** — approved 2026-09-15 at plan-sha `942a5f0c9948`, substantively revised four times since. The explainer is regenerated and in sync; that is the artifact to read. | Stale | Put the agentic program inside `agentic-label-opt`'s Docker substrate — the mount set is the boundary, and prove it with a sentinel the program cannot read plus a control that must fail. The general machinery landed in the shared engine at `592862f`, so this plan is the **consumer slice**: the pin, the mount contents, the wiring and the gates. ⚠ Nothing here is waiting on code — it waits on Phil's read, the `program-v*` tag namespace call, and then landing. |
