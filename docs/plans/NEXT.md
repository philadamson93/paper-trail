...
- **Isolation plan — CONTAINERS-FIRST decided; four-repo prior-work sweep DONE 2026-09-12.**
  [`isolation-protocol.md`](isolation-protocol.md) `Status: Draft` `Reviewed: No` · Codex review = **Blocked**, 6 criticals — **ALL APPLIED 2026-09-13** ([feedback](reviews/isolation-protocol-feedback.md)); mechanism decision adjudicated = containers first, hooks second ([feedback](reviews/isolation-mechanism-options-feedback.md)). Now **1,494 lines** (was 869). **Explain-plan feedback applied 2026-09-14:** OQ4/OQ5/OQ8 resolved (OQ3 already), leaving **5 open** — answer **OQ9 first** (does the per-run clone split into its own plan?), then **OQ1/OQ2/OQ6/OQ7**. OQ5 was the build gate and is now settled: **fail-and-feed** — a no-verdict dispatch fails the claim and is surfaced to the optimizer, no retry. OQ8 = paper-trail uses local git/disk (bucket mode is crc's). OQ4 = it's a modularity call, much belongs in the shared engine because crc needs it too. OQ1/OQ2/OQ6/OQ9 answered inline in the HTML. Explainer regenerated and on the mount at `plan-explainers/isolation-protocol.html` @ `a193c545821b`. ⚠ Rebase base moved to Plan A's tip **`f02d761`** — 447/447 reproduces there, 445 at the older `94a376f`.
