...
1. **First real scoring run — NOT RUN, and now needs a fresh cost decision.** It is the last paid gate and the only thing between here and paper-trail's v0 baseline, but at the corrected price a full VAL pass is a **~$317 / ~9-hour** commitment rather than the ~$16 the plan assumed. Phil's earlier go-ahead was given against the $32.25/iteration figure, so the 20x correction is put back to him rather than spent through. Cheaper shapes exist: a 50-claim VAL subsample is ~$50 / ~1.4h and still yields a real, if noisy, macro-F1.
