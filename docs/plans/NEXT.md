...
**Three questions stay open by design** (OQ11 adjudicator-prompt freeze, OQ12 canary frequency, OQ13 `source_mode`), each with a recommendation. **OQ13 blocks the envelope work**: the envelope wants `source_mode: "sarol_corpus"`, but that field is constrained by `verdict_schema.md`, a frozen contract file. OQ2's owed hand-sample is a rater task, not a design decision — and note that scoring both granularities *reports* more, it does not *validate* anything.

**Next, and it needs Phil.** Everything remaining is either paid or a human judgement call:
1. **Paid gates, not run** — the adapter smoke against real headless Claude Code, the live half of the budget invariant (`--max-budget-usd` demonstrably stopping an oversized run), and the first real scoring run that would establish paper-trail's v0 baseline. These spend real money and are gated on Phil's go-ahead, the same way the agentic-label-opt Phase C egress run was.
2. **`/sarol-eval-item` does not exist.** The Runner dispatches it per stage; it is named as a Task 5 eval-arm deliverable and was never built. A preflight now returns `infra_error` naming it rather than failing obscurely mid-run, but **no real run can succeed until it is written.** This is the single hard blocker on the first curve point. **The 2026-09-02 replan shrinks it**: Phase 1 needs a *single-stage adjudicator* command, not the full three-stage one, so the blocking deliverable is materially smaller than it was.
