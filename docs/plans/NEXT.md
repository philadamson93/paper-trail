...
- **Isolation plan — CONTAINERS-FIRST decided; four-repo prior-work sweep DONE 2026-09-12.**
  ...
  ✅ **Step 0a is built and green** — `optimizer/isolation.py`, 46 checks, no container and no model (`python3 experiments/sarol-2024/optimizer/isolation.py --selftest`). It caught a real plan error on first run: the snapshot cannot mount at `/workspace/program`, which the engine reserves; corrected to `/workspace/snapshot`. Negative-controlled against five injected defects. ⚠ `Reviewed` demoted to **Stale** 2026-09-16 — the plan was revised after approval. next: ⚠ **the general machinery moved upstream.** `agentic-label-opt/docs/plans/2026-09-16-contained-nested-sessions.md` (Approved 2026-09-16) now owns it, lands first, and this plan shrinks to the consumer slice — which folders, which secrets, the network list, the dry run. **Two code changes owed here, both from Phil's 2026-09-16 corrections and neither applied:** drop the per-stage mount split in `optimizer/isolation.py`, and rescope the grant from per-item to per-worker with N a parameter. ⚠ Also still owed: a force-push (the rebase diverged origin), and the engine's by-name credential form before Phase 1 can land.
