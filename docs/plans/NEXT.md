...
- **Isolation plan — CONTAINERS-FIRST decided; four-repo prior-work sweep DONE 2026-09-12.**
  [`isolation-protocol.md`](isolation-protocol.md) `Status: APPROVED 2026-09-15` `Reviewed: Yes` (approved at plan-sha `942a5f0c9948`; ⚠ the content has since moved — the two blockers below were applied after the approval read) · six review passes, **all 12 questions resolved** · scope: **adjudicator dispatch only**.
  ✅ **Both pre-coding blockers cleared 2026-09-15.** Rebased onto Plan A's tip `f02d761` — 21 commits replayed, only the two tracker files conflicted, and on the rebased branch 506/506 offline selftests, Gate A (40 checks), Gate D and the 10-entry freeze (`combined_hash 7431a5bc98a9`) all pass. Naming sweep done: **108 of 142** uses of "judge" rewritten — to *the program* where the text named the container, the mount set, the cwd or the read boundary, and to *the adjudicator* where it named the stage doing the judging (a proposed `JudgeMounts` dataclass is now `ProgramMounts`). The 34 kept are the `JUDGE_SCOPE` code symbol, the separate word *judgement*, Phil's quoted words, quoted code strings, and *Who is who*'s own discussion of the naming.
  Substance: the plan’s *Landing & cleanup* · [Codex round 2](reviews/isolation-protocol-feedback-codex-round2.md) · `docs/session/isolation-protocol-readback.md`
  ...
  · **Genuinely unbuilt = the real scope:** no program/dispatcher container anywhere (crc designed it, never built — this is what paper-trail adds); credential env vars land in the docker argv; Docker sessions write ZERO audit rows so the 4 tripwires are blind.
  ✅ **Step 0a is built and green** — `optimizer/isolation.py`, 46 checks, no container and no model (`python3 experiments/sarol-2024/optimizer/isolation.py --selftest`). It caught a real plan error on first run: the snapshot cannot mount at `/workspace/program`, which the engine reserves; corrected to `/workspace/snapshot`. Negative-controlled against five injected defects. ⚠ `Reviewed` demoted to **Stale** 2026-09-16 — the plan was revised after approval. next: ⚠ **the general machinery moved upstream.** `agentic-label-opt/docs/plans/2026-09-16-contained-nested-sessions.md` (Approved 2026-09-16) now owns it, lands first, and this plan shrinks to the consumer slice — which folders, which secrets, the network list, the dry run. ✅ **2026-09-17 — the engine landed at `592862f`** (367 tests, none skipped) and **both of Phil's 2026-09-16 corrections are now structural, not code we owe**: the engine's grant has no stage dimension, so adopting `SessionScope` drops the per-stage split, and the grant already scopes a worker's whole life with N a parameter. Phase 1's two blockers are cleared too — `inherit_env` carries the credential by name, and `render_dispatch` renders the host-allowlist egress per dispatch. ⚠ What paper-trail owes now: **pin to `592862f`** (creating our first `pyproject.toml`), delete the machinery that moved upstream, and wire the adjudicator's container. Plan revised to the consumer slice 2026-09-17; **re-read before implementing**. ⚠ Also still owed: a force-push (the rebase diverged origin), and the engine's by-name credential form before Phase 1 can land.
