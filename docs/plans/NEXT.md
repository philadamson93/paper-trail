...
- **Isolation plan — CONTAINERS-FIRST decided; four-repo prior-work sweep DONE 2026-09-12.**
  ...
  ✅ **Step 0a is built and green** — `optimizer/isolation.py`, 44 checks, no container and no model (`python3 experiments/sarol-2024/optimizer/isolation.py --selftest`; `--print-matrix` shows the rendered commands). It caught a real plan error on first run: the snapshot cannot mount at `/workspace/program`, which the engine reserves; corrected to `/workspace/snapshot`, still zero engine diff. Negative-controlled against five injected defects. next: **Step 0b's two probes (~$1)** — needs Docker plus container auth — or wire the factory into `SarolRunner.process` (1c), which Step 2 gates. ⚠ Landing Phase 1 still waits on the engine's by-name credential form (`--env NAME`); nothing is passed today, so the rendered command carries no auth yet.
