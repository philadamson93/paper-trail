...
- **Isolation plan — CONTAINERS-FIRST decided; four-repo prior-work sweep DONE 2026-09-12.**
  [`isolation-protocol.md`](isolation-protocol.md) `Status: Draft` `Reviewed: No` · Codex review = **Blocked**, 6 criticals — **ALL APPLIED 2026-09-13** ([feedback](reviews/isolation-protocol-feedback.md)); mechanism decision adjudicated = containers first, hooks second ([feedback](reviews/isolation-mechanism-options-feedback.md)). Now **1,861 lines** (was 1,494). **Second explain-plan round applied 2026-09-14 — ALL NINE open questions resolved.** Three cut scope: **OQ1** eliminates the driver *session* (Phil: it's a deterministic dispatcher, so `dispatch_prompt.py` invokes the judge directly — no Task subagent, which also retires V2i, drops `Task` from the allowlist and unblocks `--bare`); **OQ9** replaces the per-run git clone with a **run-start reset** (Phil: no parallel runs ever — ⚠ but the defect was *sequential* residue, so the reset is kept and `run_clone.py` + the `REPO_ROOT` re-resolution are struck); **OQ6** retires the tag-scoped settings file (hooks are *not* retired — still a live gap on the optimizer's `Bash` path). One adds scope: **OQ2** takes the engine change after all (`val_inputs` → `Callable[[int], RunInputs]`, mirroring `train_inputs`), so held-out staging is per-iteration and this plan **does** now touch `engine/`. **OQ7** = injected fake prefix. Earlier: OQ5 fail-and-feed, OQ8 local git/disk, OQ4 modularity. ⚠ **New `## What we are building` section** at the top — the concrete 12-item list, added because Phil asked what was actually being built. ⚠ **The explainer on the mount (`plan-explainers/isolation-protocol.html` @ `a193c545821b`) is now two rounds stale** — regenerate before the next read-through. **One detail left open:** the `program-v*` tag namespace (recommendation: key on the manifest's `combined_hash`). ⚠ **Code-audit round 2026-09-14** (4 parallel agents, both repos pulled; plan now 2,060 lines): ~20 corrections applied. Load-bearing: refusal census is **22 sites not 21** and **19 are selftest-only** — only 3 ship, and `canary.py:257` already takes an injected runner; `run_optimization`'s `components=` arg is a **third** reach-path past `build_components`; `--dangerously-skip-permissions` is set unconditionally by shared `claude_wrapper.py:130`, making deny-by-default an **upstream prerequisite** (6th engine file); and ⚠ **the 2026-08-05 armed run's `denied=0` is zero by construction** (no hooks in container mode) — now a root-cause-table row. Two engine capabilities landed the same day and retire planned work: `82f547d` LoopStop `reason` + partial-run summary (= OQ5's channel) and `ee26d80` per-version fail-closed callable (= OQ2's precedent). Evidence downgrades: the "212 passed" cite was Mac-local with the Docker tests **skipped**; negative-control suite unverified since the 2026-08-21 refactor; nested rw-inside-ro mounts fail on Docker 29.x. Engine citations now SHA-stamped at `c2dd0b3`. ⚠ Rebase base moved to Plan A's tip **`f02d761`** — 447/447 reproduces there, 445 at the older `94a376f`.
  · 📗 **[`isolation-prior-work-inventory.md`](isolation-prior-work-inventory.md) — READ FIRST.** Four-repo sweep (every branch, worktree, plan, VM readback). Its rule: **current code beats every doc, including it.** Ledger of what's BUILT/RUN vs DESIGNED, ~25 prior decisions with deciders+dates, what's genuinely unbuilt, and 7 traps.
  · **Decided (Phil):** containers for BOTH agents first, permission rules second. ⚠ Recorded **reversal** of the 2026-04-23 per-claim-Docker cost rejection (561 invocations ≈14min, not "thousands"; its own revisit-trigger fired 2026-09-09).
  · **Key corrections from the sweep:** materialised-path threading IS built (engine README 6wks stale — I cited the doc over code); rad-eval's `docker_agent.py` is on main and RAN for real ($4.13 armed iteration) — model on it, don't invent; gold-by-absence (crc, RUN GREEN 2026-08-24) is the cheapest proven pattern; **paper-trail has NO engine dependency at all**.
  · **Genuinely unbuilt = the real scope:** no judge/dispatcher container anywhere (crc designed it, never built — this is what paper-trail adds); credential env vars land in the docker argv; Docker sessions write ZERO audit rows so the 4 tripwires are blind.
  next: rewrite the plan against the sweep + Codex's 6 criticals → `/review-plan --reviewer codex` → `/read-plan`. ⚠ Branch from Plan A's **`f02d761`** (moved 5 commits); archive the 5 untracked `findings/iter-*.md` first — only copy.
...
**INSTRUMENT FIXES LANDED 2026-09-03 (`a82caaf..25c6db0`, 6 thematic commits, pushed; not merged to `sarol`).** Post-mortem items 1, 2 and 5 are done, plus the owed recoverability fix. **337 offline gates** (was 296).
- **Release payload now reaches the optimizer.** Root cause found: `engine/loop.py:378-380` writes `iter/<n>/release_{train,val}.json` **only** inside `if loop_ops is not None`, and `run_optimization` called `run_loop` without it. Fixed by passing `LocalLoopOps(repo_root)`. Negative-controlled: under the old call no `*release*` file exists anywhere, exactly as the run showed.
- **Canary fails closed.** New `optimizer/canary.py` pins a verdict by *measuring* it through the real `SarolRunner` (`--pin --repeat N` refuses an unstable judge), refuses a pin whose profile or split disagrees with itself, and `run_optimization` refuses a real run with no pin unless `--no-canary` is passed. `CostModel.canary_enabled` is now derived from whether a canary is actually wired, so "priced but absent" is unrepresentable.
- **Ramp off-by-one fixed** (`sampling.ENGINE_FIRST_ITER_N = 1` + `rung_index`), pinned by a gate that drives the **real** `run_loop` and records the `iter_n` values the factory actually receives — not by asserting the convention against itself.
- **Per-call output namespacing** (`iter<n>-current` / `iter<n>-<tag>`), so a VAL curve survives on disk; **staged-batch-size gate** on both factories *and* on static `--train-inputs`; **atomic incremental manifest writes**, so a killed run is salvageable without hand-rebuilding one.

**A `/review-implementation` (Codex) round then caught two Criticals in the fixes themselves** — see `docs/plans/reviews/papertrail-optimizer-requirements-implementation-feedback-postmortem-fixes.md`, disposition appended. Both were the *same* priced-vs-enforced shape as the original Bug 1: `BudgetGuard` did its own arithmetic and omitted the canary the preflight charged for (now delegated to one `CostModel` method), and static `--train-inputs` was handed to the engine unchecked while preflight priced `--train-n`. It also correctly called one of the new gates **decorative** — it asserted `CostModel` arithmetic while nothing asserted `BudgetGuard` carried the same term, so it stayed green under the bug it looked like it guarded. Replaced with behavioural gates, **negative-controlled: 3/3 fail under the pre-fix arithmetic**. ⚠ **Standing rule this establishes: a new guard is not done until you have watched it fail.**

**Blocking the next paid run — in this order.**
1. **Pin the canary**: `python3 experiments/sarol-2024/optimizer/canary.py --pin --profile retrieval --repeat 3` (~3 paid sessions). Until then every run refuses to start, which is intended. *Decided 2026-09-03: `--no-canary` stays reachable on any run — it reprices the run honestly and forces the waiver to be said out loud.*
2. **Metric reachability — Phil's steer 2026-09-03: hill-climbing matters more than the published comparison until a full-VAL run, and he wants 9-way macro as the target ("two classes is too weak to hill climb on").** Recomputed free from the surviving manifests: TRAIN n=50 carries 4/9 gold classes {ACCURATE 40, OVERSIMPLIFY 1, NOT_SUBSTANTIATE 8, INDIRECT 1}; VAL carries a nearly **disjoint** 4/9 {ACCURATE 29, NOT_SUBSTANTIATE 2, CONTRADICT 6, ETIQUETTE 2} — they share only two classes. Raw macro-9 ceiling is 4/9 = 0.444 on both (a *perfect* program scores that), and singleton-support classes make macro-9 ~8× noisier per claim than macro-3 (0.25 vs 0.03). **So stratification is a precondition for 9-way, not a later item.** Recommended order: stratify TRAIN and VAL onto one class set, *then* switch the frontier to macro-9 renormalised over gold-present classes (VAL is drawn once and held, so its denominator is stable for the run). Report `n_classes_present_9way` beside every number.
3. **Establish the noise floor** before reading any curve (effects are ~1 claim ≈ 0.03 macro-F1 at n=37, so step-back would be driven by noise; adopt crc's `τ_cal`). Note `step_back_revert_tolerance` defaults to `0.0` and is never set — see the prompt review below.
4. **Act on the optimizer-prompt review** — `docs/session/papertrail-optimizer-prompt-review.md` (git-ignored). The instruction set promises mechanisms this consumer does not implement: `followups` is documented and emitted by nothing; the step-back declaration is hard-coded `False` (`dispatcher.py:417`, `adapter.py:1143`); "change note" names a non-existent artifact; the per-claim call-count ceiling is unenforced and unreachable under Phase 1; and the four `context/` paths in `optimizer-instructions.md` do not resolve from the agent's cwd. **Prompt edits are the experiment's independent variable — Phil's call, not a session's.**
**Longer-tail items, not blocking the next run:**

1. ~~**`/sarol-eval-item` does not exist.**~~ **WRITTEN 2026-09-02** (`a1fb578`), at `.claude/commands/sarol-eval-item.md`. Single-stage, as the replan scoped it: the `adjudicator` stage is implemented; `extractor`/`verifier` abort with `STAGE_NOT_IMPLEMENTED` pointing at the C6.1 profile retrofit, deliberately, so a missing profile is visible rather than quietly running Phase 2. ⚠ **Location is load-bearing** — `command_path()` accepts three directories but Claude Code only resolves `.claude/commands/` from the session cwd; the other two pass the preflight and then fail at dispatch. An adapter selftest pins the location. ~~The remaining blocker on a first curve point is now the Part C6 retrofit.~~ **That retrofit LANDED 2026-09-02** — the BM25 producer writes the envelope `/sarol-eval-item` reads, and `STAGES` is profile-derived rather than a global 3-tuple. The command was exercised for real in the adapter smoke above.
