...
- **Isolation plan — CONTAINERS-FIRST decided; four-repo prior-work sweep DONE 2026-09-12.**
  ...
  next: **implement Phase 0a** — the free dry run: render every command for every stage/profile/claim/version combination, start no container, call no model, and assert no host path leaks and no two dispatches share a command. ⚠ The explainer (`isolation-protocol.html`) is now stale against the swept plan — regenerate with `/explain-plan` before any visual review.
