...
8. **Declarative per-subagent heterogeneous controllability as an optimization surface** — via Claude Code subagent YAML config (`.claude/agents/*.md`: `model`, `tools`, `disallowedTools`, `mcpServers`, `permissionMode`, `memory`, `skills`, `isolation`). Combined with topology-as-search-space (D32). No surveyed system treats this specific surface as optimization variable: AFlow has per-node config in flat graph (homogeneous nodes, not per-subagent-role heterogeneity); VeRO fixes target model to GPT-4.1-mini across all 5 benchmarks; Meta-Harness is monolithic single-file Python; autoresearch is monolithic `train.py`. **Narrower substrate argument than "Claude Code is special":** specifically, declarative YAML per-subagent config is easier to vary and commit than imperative per-call Python config, making per-subagent heterogeneity first-class. Directly answers **VeRO's empirical negative finding** (*"current optimizers default to prompt modifications, exhibiting limited diversity and impact in the changes they can produce"*) — our substrate choice makes architectural edits first-class by design.
