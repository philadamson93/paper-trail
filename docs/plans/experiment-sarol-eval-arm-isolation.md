# Eval-arm invocation isolation (Rule 3)

**Companion to** `experiment-sarol-optimization-loop-hygiene.md` (Rules 1 subagent-sandboxing + 2 main-session-test-blindness). **Split from that doc 2026-04-23** when the IN/OUT formalization + canonical invocation shape + alternatives-evaluated catalog + residual-investigation items grew too large to coexist with the original optimization-loop-hygiene rules.

**Empirical record:** `docs/journal/2026-04-22-memblind-oauth-findings.md` (six canaries + critic audit); `docs/journal/2026-04-23-benchmark-integrity-lit-review.md` (Berkeley Agent-Eval Checklist + Zhu et al. ABC synthesis + creative-defenses brainstorm).

**Companion runbook:** `docs/plans/canary-runbook-vertex.md` (landmark-tier Vertex canary execution plan, pending).

---

## Rule 3 — eval-time IN/OUT isolation
...
- **Project-level machine-local `.claude/` files** (added 2026-04-23 after a concrete incident where an OAuth token got cached into `.claude/settings.local.json` as an approved-Bash-command-pattern during a canary test session). Everything gitignored under `.claude/` is by definition host-specific, including:
  - `.claude/settings.local.json` — per-user approved-command + permission cache. May contain command-line strings that include secrets (as happened 2026-04-23 with a `CLAUDE_CODE_OAUTH_TOKEN` leak). Never flows into eval — fresh `CLAUDE_CONFIG_DIR` suppresses it structurally, but the file itself remains on disk and is a local-hygiene concern separate from eval-invocation isolation.
  - `.claude/skills/*` — auto-installed third-party skills (e.g., paperclip). Project-owned skills are committed explicitly (carve-out in `.gitignore`); everything else here is host-specific.
  - `.claude/scripts/__pycache__/` — transient Python byte-code.
  The general class: **anything the `.gitignore` file excludes under `.claude/` is OUT-of-system by definition** — the project's `.gitignore` is the canonical boundary between IN-system (committed, tag-scoped) and OUT-of-system (host-local, session-local, or secrets). Evaluators should treat `.gitignore` as the authoritative OUT-of-system manifest for the `.claude/` subtree.
...
## Mechanisms (two-tier; verified 2026-04-22, amended 2026-04-23 post-benchmark-integrity-lit-review)
...
### Iteration tier — internal bash that the wrapper script runs
...
### Landmark tier — internal bash inside the Docker container
...
### Binary version invariant (added 2026-04-22 post-critic)

Every archived `paper-trail-v<N>` eval run must record the Claude Code binary version (`claude --version` captured at run time) in its `summary.json` / archive metadata. Landmark-tier runs SHOULD pin the binary version via `npm install -g @anthropic-ai/claude-code@<pinned>` on the eval host (or via the Dockerfile FROM line); iteration-tier runs record version without pinning and accept version-drift-within-iteration as an acknowledged residual. A binary-version change across a landmark-tier curve requires re-baselining.

## Relationship to Rules 1 and 2

- **Rule 1** (`optimization-loop-hygiene.md`) applies inside each `claude --print` invocation: the subagents dispatched by paper-trail's internal Agent-tool calls are sandboxed from gold labels. Unchanged.
- **Rule 2** (`optimization-loop-hygiene.md`) applies to the optimizer's read access to val / test splits. Structurally enforced via Tier 3 sealing in the agent-only framework. Unchanged.
- **Rule 3** (this doc) applies to every `claude` subprocess the optimizer spawns — it controls what auto-discovered context enters each invocation regardless of Rule 1 / Rule 2.
