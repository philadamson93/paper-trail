...
**Extracted to its own doc 2026-04-23** when the formalization grew past the scope of optimization-loop hygiene. See `experiment-sarol-eval-arm-isolation.md` for the full content: IN/OUT formalization, canonical iteration-tier and landmark-tier invocation patterns (wrapper script + Docker), per-flag mechanism rationale, alternatives-evaluated catalog with "why didn't he just..." defense material, and residual investigation items with priority.

**Relationship in one sentence:** Rules 1 and 2 (this doc) govern what labels and test data the optimizer and its subagents can see; Rule 3 (sibling doc) governs what host-level context the per-claim `claude --print` invocations auto-discover into themselves from the eval host.
