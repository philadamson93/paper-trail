...
- **Author-mode invocation smoke (RD-9).** Execute the five-step "Author-mode invocation" procedure against `examples/DFD_authormode/main.tex`: invoke `/paper-trail --author /abs/path/to/main.tex` once from inside paper-trail's dir and once via `claude --add-dir` from `/tmp`. **Pass criterion (revised 2026-06-24 after the smoke ran): path resolution + output routing are cwd-independent** — both invocations must resolve paper-trail's repo root (`git rev-parse --show-toplevel`) to the same canonical path, write only under the resolved output dir, and leave the manuscript untouched. **Byte-identical artifacts are *not* a valid criterion** (the original "produce identical outputs" framing): the extraction → fetch → grounding pipeline is LLM-driven and stochastic, so two independent runs legitimately differ in claim_text phrasing, fetch strategy, validator FLAG counts, and stub-schema population — all observed in the 2026-06-24 run, none cwd-related. Mandatory before reorg ships — author mode is a public API surface. Replaces the prior vendoring smoke; there is no vendored copy to verify.
...
`/review-implementation` ran after the initial commits (verdict: Revise before commit) — feedback + adjudication log at `reviews/repo-organization-implementation-feedback.md`. Applied: dotted `{{claim_type_hint.*}}` slots added to the `control_flow.md` slot map (the original enumeration grep missed dotted placeholders — command fixed in-file), validation cells converted to real Markdown links per the no-drift constraint, and verifier result semantics extracted to a new `src/specs/verifier_results.md` (user's call) with `verifier-dispatch.md` / `paper-trail.md` / `control_flow.md` pointing at it. Open: whether the documented author-mode default-output set includes `claims_ledger.md` (currently documented). The 2026-06-24 author-mode smoke used `--output-dir` overrides, so the *default* path was not exercised — still requires a default-output run to settle (see "Behavioral smoke results" below).

### Behavioral smoke results (2026-06-24, headless Claude Code sessions)

Run by a background agent via `claude -p` sessions rooted in the `repo-organization` worktree (not the post-merge main checkout, but the symlink layout under test is byte-identical). **Discovery + execution-time path resolution PASS; end-to-end grounding was environment-blocked** (orthogonal to the reorg — see caveat).

1. **`/paper-trail` reader-mode end-to-end** (`examples/paper-trail-adamson-2025/input-paper.pdf`, scratch `--output-dir`) — **execution path PASS, grounding blocked.** Extracted 86 claims, fetched 15 reference PDFs, wrote `ledger/claims/*.json` + `refs.bib` + `parse_report.md` through the symlinks with **zero path-resolution errors**. Did not reach the Phase-5 `demo.html` render — the run died on a transient `API Error: Overloaded` (infra flake, retryable). All claims `PENDING/NEEDS_PDF` (ingest unavailable — see caveat).
2. **Per-command + skill discovery** — **PASS.** A headless session in the worktree discovered all six slash commands (`/fetch-paper`, `/ground-claim`, `/init-writing-tools`, `/paper-trail`, `/paper-trail-init`, `/verify-bib`) and all three skills through the symlinks — both shapes: single-file (`plan-check`, `doc-split-check`) **and** directory-shaped (`paperclip/SKILL.md`). Closes the structural half of smokes 2–3.
3. **Author-mode two-cwd (RD-9)** — **path/routing contract PASS.** Run A (cwd = paper-trail dir) and Run B (cwd = `/tmp` via `claude --add-dir`) both resolved paper-trail's repo root to the **identical canonical path** via `git rev-parse --show-toplevel`, validated the absolute manuscript path, invoked the real `src/scripts/*.py`, wrote only under `--output-dir`, and left the manuscript untouched. **`--add-dir` does register the slash command** (the open half of RD-9's two-cwd discovery question). Outputs were **not** byte-identical (A: 6 refs fetched / 100 surviving claims / 3 FLAG; B: 0 fetched / 103 claims / 5 FLAG) — pure LLM nondeterminism, not cwd-dependent; this is why the smoke's pass criterion was revised (see "Smoke test plan" → author-mode invocation smoke).

**Environment caveat (blocks grounding everywhere, independent of the reorg).** This Mac has no reference-fetch path (paperclip CLI absent; `paper-search`/`papersflow` MCP absent) and no ingest tooling (`pdftotext`, GROBID, `lxml` all missing), so Phase 2.5 ingest cannot build a source handle and every claim lands `PENDING/NEEDS_PDF`. The extractor → adjudicator → verifier grounding loop is therefore **not exercised** by these runs; it needs a machine with the ingest stack and a fetch path, can be smoked separately, and **does not gate the merge** (it is not a reorg-path concern).

**Still open (needs a fuller environment, not merge-blocking):** a clean reader-mode run through the Phase-5 render (retry past the API overload), and a *default-output* author run (no `--output-dir`) to settle whether the default set includes `claims_ledger.md`.
