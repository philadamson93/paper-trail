...
**Topology-restructure invitation (added 2026-04-23).**

The catalog above is *permissive* about topology restructure — splitting/combining prompts and creating new ones implies the optimizer *may* restructure subagent decomposition. Permission-by-implication is not enough: the seed prompt should explicitly *invite* topology exploration as a recommended-to-attempt proposal class, distinct from the cheaper prompt-text-edit moves the optimizer will reach for by default. Working draft language to land in the seed prompt (`program.md`-equivalent), not yet authoritative:

> *"Topology restructure is a first-class proposal worth trying at least once or twice during the optimization loop — the number of subagents, their roles, and how they hand off are explicit knobs you should consider changing, not a default decomposition you should defer to. Prompt-text edits within the current decomposition are cheaper to evaluate, and you should generally prefer them, but at least one substantial topology-restructure proposal should be evaluated before declaring the seed decomposition near-optimal."*

**Rationale.** A key limitation we are prepared to acknowledge in the paper (see `paper-writeup-items.md` §"Things to be honest about" — topology-may-not-change item) is the empirical outcome where the optimizer never proposed any alternative topology, leaving us unable to distinguish "seed was near-optimal and the optimizer rationally backtracked from a tried alternative" from "the optimizer was biased by the seed prompt against trying restructure at all." Inviting exploration explicitly raises the chance the realized outcome is the former (evidence FOR the framework claim) rather than the latter (neutral-to-negative).

**Companion requirement — proposed-but-rejected logging.** Inviting exploration is necessary but not sufficient. The per-revision archive artifact must capture restructure proposals the optimizer made-evaluated-and-rejected, not only accepted changes. Without this logging, an "explored-and-backtracked" outcome is indistinguishable from a "never-explored" outcome at the artifact level. Folds into §7 open-problem-#10's archive schema; tracked as a Task-5 deliverable in NEXT.md tier-1.
...
10. **What counts as "the optimizer agent"?** Its system prompt, its tools, its `--add-dir` scope, its initial context — all of these are Tier 1 invariants for reproducibility. Needs a schema (folds into the `paper-trail-v<N>.json` archive artifact). **Partial spec committed 2026-04-22** — see §3 "Optimizer agent initial configuration" (affordance catalog, performance-not-cost philosophy, fight-Python-default guidance). Full machine-checkable schema still owed. **Added 2026-04-23 — schema must include proposed-but-rejected topology changes:** per-revision archive artifact captures *attempted* restructure proposals (and the eval results that caused them to be rejected), not only accepted changes. Required to support the explored-and-backtracked vs never-explored distinction when the realized end-of-experiment topology equals the seed topology — see `paper-writeup-items.md` §Things-to-be-honest-about (topology-may-not-change item) for the framing argument and §3 "Topology-restructure invitation" subsection above for the companion seed-prompt invitation.
...
**Bucket 5 — Benchmark-integrity prior art (added 2026-04-23).** Prior work on hardening agentic benchmarks themselves against scoring bugs, ground-truth leakage, and agent-vs-evaluator contamination. Our substrate-level-isolation contribution extends these frameworks to a new axis (the execution runtime itself as a leakage surface).

- **Zhu et al. 2025** — "Establishing Best Practices for Building Rigorous Agentic Benchmarks" (arxiv 2507.02825, 25 authors including Kapoor, Longpre, Steinhardt, Zaharia, Stoica, Liang, Kang). The canonical academic reference; introduces the Agentic Benchmark Checklist (ABC). Reports representative failure modes including SWE-bench Verified's insufficient test cases and TAU-bench's "empty responses counted as successful" bug, with documented performance-overestimation up to 100%.
- **Wang et al. 2026** — "How We Broke Top AI Agent Benchmarks" (UC Berkeley RDI blog, April 2026). Hao Wang, Qiuyang Mang, Alvin Cheung, Koushik Sen, Dawn Song. Empirical adversarial audit of 8 major benchmarks (SWE-bench, WebArena, OSWorld, GAIA, Terminal-Bench, FieldWorkArena, CAR-bench, SWE-Lancer) showing every one can be exploited to near-perfect scores without solving tasks. Introduces BenchJack automated scanner and a 7-pattern vulnerability taxonomy plus Agent-Eval Checklist.
- **Fan et al. 2025** — "Fault-Tolerant Sandboxing for AI Coding Agents: A Transactional Approach to Safe Autonomous Execution" (arxiv 2512.12806). Transactional-filesystem sandboxing pattern for coding agents; candidate mechanism for paper-trail's PDF-fetch cache state management in a future-work paper.
- **METR** — 2025-06-05 blog post on frontier-model reward hacking (o3: 30.4% of HCAST/RE-bench runs showed reward-hacking attempts; 70-95% even when explicitly instructed not to). Prior empirical motivation for the round-trip sanity canary (our D46) and for Tier 2's "never show the optimizer per-example val data" discipline.

**Differentiation vs Bucket-5 entries:** Zhu + Wang both target evaluator-side integrity (benchmark designers should harden scoring, sandbox the agent from ground truth, structure outputs, test with null/random/injection/tamper adversarial agents). Our novel axis is *substrate-side integrity* — the execution runtime (Claude Code as both orchestrator and agent substrate) leaks host state (CLAUDE.md / auto-memory / hooks / plugins / Claude Code binary version / dynamic system prompt / session history / shell env) in ways that don't fit the agent-vs-evaluator frame. Our IN/OUT formalization (`experiment-sarol-eval-arm-isolation.md`) + two-tier architecture (iteration + landmark, with wrapper-script and Docker respectively) + committed "alternatives evaluated" table is the extension. We adopt their evaluator-side recommendations (adversarial smoketests, scorer audit, crashed-task discipline, read-only filesystems, structured outputs) as Task-5 deliverables — details in `docs/journal/2026-04-23-benchmark-integrity-lit-review.md` §3-§4.

Full decision log and required-citation list: journal `docs/journal/2026-04-22-lit-review-2-competitor-landscape.md` D41 and `docs/journal/2026-04-23-benchmark-integrity-lit-review.md`.
