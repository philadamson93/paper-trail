...
**Evidence-item shape (⚠ corrected against the live schema — Codex review 2026-06-24).** The plan previously described an `attestation.hits[]` model that does not exist. The live `verdict_schema.md` (1.0) has: per-sub-claim `evidence[]` = `{section, line, snippet}` (line refers to `handle/content.txt`); a *separate* top-level `attestation` object holding a `section_checklist` (not a hits array); `figures_checked[]` = `{figure, question, vision_response}`; and `source_ref_urls[]`. The 1.1 bump must carry paperclip provenance **on the evidence item**, and the exact shape is an open decision — see OQ6. Lighter-touch option (recommended): generalize the evidence item to `{section, line, snippet, source_mode?, locator?}`, where `locator` is `/papers/<handle>/sections/<name>.lines#L<n>` (paperclip) or `pdfs/<citekey>.pdf#page=<n>` (PDF), so every existing consumer (adjudicator, verifier, renderer, ledger) keeps reading `{section, line, snippet}` and only the new fields are additive. `figures_checked[]` gains an optional `figure_path` for the `/papers/<handle>/figures/<file>` form.
