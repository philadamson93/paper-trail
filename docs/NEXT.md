...
3. **[Paperclip-first architecture](plans/feature-paperclip-first-architecture.md)** — paperclip-mode read-path with PDF fallback; schema 1.0 → 1.1 adds `source_mode`/`paperclip_handle`/`locator`. **Phases 0-2 + Phase-3 (schema 1.1 `8d2019b` + dispatch `0f25bd2`) + Phase-5 render (source-mode badge + `claims_ledger.md` column) all SHIPPED and Codex `/review-implementation`-reviewed 2026-06-27. SKILL handling resolved 2026-06-27 (v0.5.11 stub adopted + dispatch repointed to `paperclip skill`). Remaining: end-to-end smoke (Steps 0-5) — needs the live paperclip CLI + auth/network + `pdftotext`/GROBID.** In flight on `feat/paperclip-first-smoke-gate`.
...
| feature-paperclip-first-architecture | In flight (Phases 0-2 + Phase-3 `8d2019b`/`0f25bd2` + Phase-5 render + SKILL-stub adoption shipped, Codex-reviewed 2026-06-27; remaining: end-to-end smoke, needs live paperclip CLI) | No | `feat/paperclip-first-smoke-gate` |
