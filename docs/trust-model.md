...
- **Pre-dispatch claim invariants.** Before any LLM grounding runs, a non-LLM validator checks each extracted claim: its text must actually appear in the manuscript (`TEXT_ANCHOR_MISSING`); its anchor must fall in the paper body, not the title/author/affiliation block where superscript affiliation digits look identical to numbered citation markers (`FRONT_MATTER_ANCHOR`); and the citation markers within ±280 chars of the anchor must include the cited refnum (`CITEKEY_MARKER_MISMATCH`). Flagged claims are dropped, re-extracted, or escalated — never silently dispatched.
