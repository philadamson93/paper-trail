...
- `stage` = `"adjudication"`
- **Copy every top-level field of the evidence file into your output verbatim, then change only what this contract tells you to change.** The exit validator requires all eleven of `claim_id`, `schema_version`, `run_id`, `citekey`, `source_mode`, `claim_text`, `claim_type`, `attestation`, `stage`, `sub_claims`, `overall_verdict`; dropping any one of them fails the whole file (`MISSING_FIELD:<name>`) and the claim scores as a miss whatever your verdict was. Copy `attestation` entire, including `phrasings_tried` — an emptied or rebuilt `attestation` also trips `PHRASINGS_FLOOR`. `source_mode` must be **top-level**: its presence inside `evidence[*]` entries does not satisfy the requirement (`SOURCE_MODE_MISSING`). `handle`, `paperclip_handle` and `ingest_mode` carry across unchanged too
- **Emit valid JSON.** Carried-forward snippets are verbatim source text and routinely contain double quotes, backslashes and newlines; any one of them can make the file unparseable, and an unparseable file scores as a miss whatever your verdict was. This is a real and recurring loss. **Neutralise those characters rather than escaping them.** Before writing a snippet, replace every double-quote character in it with an apostrophe, replace every line break with a single space, and drop any backslash it contains. **The finished file must therefore contain no backslash character anywhere** — that is the whole rule, and checking it is the last thing you do before you write. A snippet is provenance, not a transcript: changing its punctuation costs nothing and losing the file costs the claim. You may also shorten a long snippet to the span that actually matters
